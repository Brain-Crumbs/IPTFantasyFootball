...
BOOT-025 adds `control-plane.controlled-merge`, the only supported path that merges a task's merge-ready pull request and transitions it `MERGE_READY -> MERGED -> DONE`. `ControlledMergeController.merge()` holds an exclusive per-task lock across its entire read-decide-write critical section (mirroring BOOT-018's/BOOT-019's/BOOT-020's/BOOT-021's own file-based task locks, extended here to span the module's async provider calls) and dispatches on the task's *persisted* lifecycle state: a `MERGE_READY` task re-evaluates the unmodified BOOT-024 `MergeReadinessPolicyEngine.evaluate()` itself immediately before merging and re-checks the pull request's actual remote head one more time immediately before calling the merge provider, rejecting any drift in either gap as `HEAD_CHANGED`; a pull request already discovered `merged: true` is trusted only when the task's own lifecycle history still binds its `MERGE_READY` transition to the exact current revision and the merged pull request's base matches the configured integration target, closing the shortcut a prior interrupted attempt's own confirmed merge would otherwise need without re-deriving BOOT-024's other checks unsafely; a `MERGED` task (evidence and the `MERGED` transition already persisted, but lock release/`DONE` interrupted) resumes with zero provider calls; and a `DONE` task returns its persisted evidence idempotently. Every confirmed merge is recorded as a new `ipt.merge-evidence` record (a third, purely additive schema/lineage BOOT-015's evidence store now serves) before the `MERGED` lifecycle transition, so every resume path has a well-defined source of truth; the assignment lock is released as a best-effort step that only ever touches the exact lock identity observed when `merge()` began, never a lock some other actor has since legitimately reacquired. It performs no QA/Architecture/UAT/MergeController judgment itself, executes no merge without a confirmed BOOT-024 `ready: true` (or an already-confirmed provider result), and overrides no failed gate; general release/deployment automation and CLI/orchestration wiring remain owned by BOOT-026 onward.
