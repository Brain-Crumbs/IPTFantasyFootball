...
- **Current implementation task: BOOT-024 — Merge-readiness policy engine / issue #26**
- Canonical BOOT-024 branch: `bootstrap/boot-024-merge-policy`
...
BOOT-024 adds `control-plane.merge-readiness`. `MergeReadinessPolicyEngine.evaluate()` resolves the task's canonical branch and exact current revision through the unmodified BOOT-011 Git branch adapter, reads current per-role approval evidence through the unmodified BOOT-021 `getApprovalStatus()` (rejecting a revision changed mid-read as `REVISION_CHANGED`, exactly like BOOT-022), reads the findings on any role's current non-`PASS` review-result through the unmodified BOOT-015 evidence store, checks every declared dependency's persisted lifecycle state for exactly `DONE`, queries the required BOOT-023 CI check names' latest run for the exact revision through a new `GitHubCiStatusOperations` GitHub-Checks-API adapter, and discovers the canonical open pull request for that branch/`main` through the same `PullRequestRecord` shape BOOT-022 defines. It returns a `ready` boolean that is exactly `reasons.length === 0` plus a typed, machine-readable `reasons` array — one entry per unresolved review, unresolved blocking finding, unsatisfied dependency, failing/missing CI check, or pull-request head/base/identity mismatch — with no request field or free-text override capable of forcing `ready: true` over any one of them. It performs no QA/Architecture/UAT/MergeController judgment (a role's already-recorded outcome is read back, never re-decided), executes no merge, and mutates no lifecycle state or evidence; those remain owned by BOOT-025 onward. No CLI command is added.
...
- controlled merge execution and completion transition (BOOT-024 computes merge readiness as a read-only query; it does not merge or transition lifecycle state — see `contracts/merge-readiness/README.md` "Out-of-scope follow-up");
- agent provider adapters/runners;
- sequential orchestration/cutover tooling;
- fantasy-football product behavior.

Later BOOT issues own those capabilities and must not be pulled into BOOT-013, BOOT-016, BOOT-018, BOOT-019, BOOT-020, BOOT-021, BOOT-022, BOOT-023, or BOOT-024.
