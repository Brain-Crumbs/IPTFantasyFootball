...
- BOOT-025 — Controlled merge and completion transition
- **Current implementation task: BOOT-026 — Agent runner/provider interface / issue #28**
- Canonical BOOT-026 branch: `bootstrap/boot-026-agent-runner-interface`
...
BOOT-026 adds `control-plane.agent-provider`, a provider-neutral contract for invoking one AI/automation role session so orchestration is never coupled to ChatGPT, Codex, or any other single vendor. `AgentProvider` is the pluggable port a real vendor adapter implements later (none ships here); `AgentRunner.run()` wraps an injected `AgentProvider`, rejects a request whose `contextPackage` does not match its own `taskId`/`role`/`revisionIdentity` (mirroring `ReviewFramework.submit()`'s identical check) or whose role the provider does not support, enforces `timeoutMs`/`AbortSignal` cancellation itself via a real timer race rather than trusting the provider to self-enforce either, and normalizes every provider failure into one typed `AgentProviderError` with a stable `code` and `recoverable` flag. `AgentRunResult` reuses `control-plane.review-framework`'s own `ReviewOutcome`/`ReviewFinding`/`ReviewNonPassDetail` types directly, so a future orchestrator can build a `ReviewSubmissionRequest` from it without loss. `FakeAgentProvider` is the in-memory, network-free test provider this module ships for exercising all of the above. It invokes no real AI vendor and performs no sequential multi-role orchestration; those remain owned by BOOT-027 onward. No CLI command is added.
...
- real agent-provider vendor adapters (BOOT-026 defines the provider-neutral `AgentProvider`/`AgentRunner` interface and an in-memory `FakeAgentProvider` for tests only; no ChatGPT/Codex/Anthropic SDK integration exists);
- sequential orchestration/cutover tooling;
- fantasy-football product behavior.

Later BOOT issues own those capabilities and must not be pulled into BOOT-013, BOOT-016, BOOT-018, BOOT-019, BOOT-020, BOOT-021, BOOT-022, BOOT-023, BOOT-024, BOOT-025, or BOOT-026.
