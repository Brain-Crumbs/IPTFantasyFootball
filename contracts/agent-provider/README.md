...
- `run()` validates the request itself before touching the provider: `taskId` must match `^[A-Z]+-[0-9]{3,}$`; `role` must be one of `AGENT_RUNNER_ROLES`; `revisionIdentity`/`runId`/`actorId` must be non-empty and already trimmed; `contextPackage` must be an object with string `taskId`/`role`/`sourceRevision` fields (checked before any of them is ever dereferenced, so a missing/`null`/malformed `contextPackage` is `INVALID_REQUEST` rather than a raw `TypeError`); `toolPermissionPolicy.allowedTools`/`deniedTools` (when present) must be arrays of non-empty trimmed strings; `toolPermissionPolicy.networkAccess` must be one of `AGENT_NETWORK_ACCESS_LEVELS`; `timeoutMs`, when present, must be a positive integer no greater than `2147483647` (`setTimeout`'s own 32-bit delay ceiling — a larger value would otherwise silently truncate to an almost-instant spurious timeout); `signal`, when present, must expose a boolean `aborted`. Any violation is `INVALID_REQUEST` (`recoverable: false`).
- Immediately after that validation, and before the provider is ever called, `run()` freezes the request object itself (`Object.freeze`). A provider that then tries to reassign a top-level field (most importantly `taskId`/`role`/`revisionIdentity`/`runId`) on the exact object it was handed either throws (in the strict-mode context every ES module runs in) or silently no-ops; either way, every later read of that identity in this call — including the final result-binding check below — always reflects the identity `run()` already validated, never a value a nonconforming provider rewrote in place before resolving.
...
- `run()` reads the provider's `capabilities()` and rejects a result that is not an object with an array-valued `supportedRoles` as `PROVIDER_ERROR` (`recoverable: true`) — a malformed capabilities shape is a provider fault, not a caller mistake, so it is normalized the same way a thrown error would be rather than throwing a raw `TypeError` out of the `.includes()` check. Once `supportedRoles` is confirmed to be an array, a `role` absent from it is `UNSUPPORTED_ROLE` (`recoverable: false`), before the provider's `run()` is ever called. A raw throw from `capabilities()` itself is normalized to `PROVIDER_ERROR` the same as a throw from `run()`.
...
- After the race settles successfully, the raw result is validated before ever being returned: it must be a non-null, non-array object whose `taskId`/`role`/`revisionIdentity`/`runId` exactly equal the request's own, whose `providerId` equals the provider's own `providerId`, whose `outcome` is one of `PASS`/`FAIL`/`BLOCKED`, whose `details` is an object, whose `findings` is an array of well-formed, uniquely-`findingId`'d `ReviewFinding`-shaped entries (non-empty `findingId`, a recognized severity, non-empty `observed`/`expected` — a duplicate `findingId` is rejected the same way `ReviewFramework.submit()` would reject it later, per `src/review-framework/review-framework.ts:364-371`), whose `evidenceRefs` is an array of strings, whose `occurredAt` is a parseable date-time string containing `T`, and — when `outcome` is not `PASS` — whose `nonPass` carries non-empty `reason`/`remediation` strings. Any violation, including a value that cannot pass through `structuredClone()`, is `MALFORMED_RESULT` (`recoverable: false`).
- A validated result is never handed back as a shallow-frozen view over the provider's own data: it is deep-cloned via `structuredClone()` and then recursively frozen before `run()` returns it, so neither the provider mutating its own `details`/`findings`/`evidenceRefs`/`nonPass` afterward, nor a caller mutating the returned result's nested fields directly, can change an `AgentRunResult` once it has been returned.
...
- A request object's `taskId`/`role`/`revisionIdentity`/`runId`, once validated at the start of `run()`, can never be observed to change for the remainder of that call, whether by the wrapped provider or any other holder of the same reference.
- An `AgentRunResult` returned from `run()` is always independent of the provider's own objects/arrays and always deeply immutable; no caller or provider can mutate a field of an already-returned result.
...
- A provider whose `capabilities()` resolves without throwing but returns a shape missing an array-valued `supportedRoles` is normalized to `PROVIDER_ERROR` rather than throwing a raw `TypeError` out of `run()`'s own role-support check.
- A request whose `contextPackage` is missing, `null`, or an object missing a string `taskId`/`role`/`sourceRevision` field is rejected as `INVALID_REQUEST` before any of those fields is ever dereferenced.
- A request whose `timeoutMs` is a positive integer at or above `2147483648` (one past `setTimeout`'s 32-bit ceiling) is rejected as `INVALID_REQUEST` rather than silently producing an almost-instant spurious `TIMEOUT` once passed to `setTimeout`.
- A provider result whose `findings` array contains two entries sharing the same `findingId` is rejected as `MALFORMED_RESULT`.
- A provider that mutates the request object it was handed (for example reassigning `request.taskId`) before resolving does not thereby change what identity the returned `AgentRunResult` is validated against; the request is frozen the moment `run()` finishes its own validation, before the provider is ever invoked.
