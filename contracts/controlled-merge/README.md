...
Final release (once `fn` completes, in `withLock`'s own `finally`) uses the same atomic-claim technique as `reclaimIfStale`, not a plain read-then-unlink: it renames the lock path away first, verifies the *claimed* content is genuinely this holder's own token, and only then discards it — restoring the claimed content untouched if it turns out to belong to someone else (most likely a reclaimer's own fresh replacement). A plain read-then-unlink cannot rule out that exact race: a reclaimer could rename the old lock away, verify it, and write its own fresh replacement back to the same path in the gap between this holder's read (which still observed its own, soon-to-be-stale token) and its `unlinkSync` call — which would then delete the *replacement's* lock, not this holder's own already-gone one, letting a third caller's `tryCreate()` succeed as if the task were unlocked while the replacement's own callback is still actively running.
...
- `ControlledMergeLockStore.get(taskId)/release(request)` — satisfied by `control-plane.assignment-lock`'s `FileAssignmentLockStore`, extended (additively) with an optional `expectedOwnerId`/`expectedRunId`/`expectedCanonicalBranch` compare-and-swap guard on `release()`. This controller's own pre-call check (comparing the full captured `LockIdentity` against a freshly re-read record) and the actual `release()` call are two separate operations with a gap between them; passing the captured identity's `ownerId`/`runId`/`canonicalBranch` as that guard closes the gap atomically — a stale-recovery that reuses this exact `lockId` for a different, legitimate owner in that gap (permitted by the assignment-lock contract) is rejected as `LOCK_ID_MISMATCH` by `release()` itself, not merely by this controller's own earlier, racy check. Both `get()`/`release()` calls are individually normalized: a raw throw from either `get()` (at merge entry or at completion-time re-read) or `release()` itself surfaces as a recoverable `ControlledMergeError("LOCK_RELEASE_FAILED", ...)`, never an unhandled exception.
