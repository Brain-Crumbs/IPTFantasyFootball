...
No CLI command is wired to this module's behavior. `agent review` was already registered as a reserved command by BOOT-017; this module additionally registers `agent rework` as reserved (`RESERVED_COMMANDS` in `src/cli/commands.ts`), so invoking either today deliberately fails with the CLI's `NOT_IMPLEMENTED` exit code rather than `USAGE_UNKNOWN_COMMAND` — both remain reserved for a later BOOT task (BOOT-026 onward) to wire to the role-specific and rework workflows this module and BOOT-018/019/020 already implement.
...
4. parse the `<ROLE>_FAILED` history event's own `evidenceRef` (`lineageId@sequence`) and independently read that role's review-result lineage back from the unmodified BOOT-015 evidence store, requiring its *current* record to still be exactly that sequence, bound to the exact revision, and not `PASS` — the lifecycle history and the evidence store are each verified on their own, neither trusted as proof of the other, and rework is never bound to "whatever the lineage's current record happens to be" if a later same-role attempt (which `control-plane.review-framework` permits) has since superseded the record the failure actually referenced;
