...
test("FileArchitectureReviewTaskLock never races a second caller reaching the same stale conclusion against a still-live rollback-recovery claim", () => {
  const root = mkdtempSync(join(tmpdir(), "ipt-architecture-review-lock-live-rollback-recovery-claim-"));
  try {
    const lock = new FileArchitectureReviewTaskLock(root);
    const lockPath = join(root, "BOOT-019.lifecycle.lock");
    const rollbackClaimPath = `${lockPath}.try-create-rollback-claim`;
...
test("FileArchitectureReviewTaskLock resumes an abandoned rollback-recovery claim (crashed after claiming, before finishing) rather than wedging the task forever", () => {
  ...
  const root = mkdtempSync(join(tmpdir(), "ipt-architecture-review-lock-abandoned-rollback-recovery-claim-"));
  try {
    const lock = new FileArchitectureReviewTaskLock(root);
    const lockPath = join(root, "BOOT-019.lifecycle.lock");
    const rollbackClaimPath = `${lockPath}.try-create-rollback-claim`;
...
test("FileArchitectureReviewTaskLock fully drains a double-orphaned rollback claim (crashed between linking and removing the original) across successive acquisitions, never losing the displaced content", () => {
  ...
  const root = mkdtempSync(join(tmpdir(), "ipt-architecture-review-lock-double-orphaned-rollback-claim-"));
  try {
    const lock = new FileArchitectureReviewTaskLock(root);
    const lockPath = join(root, "BOOT-019.lifecycle.lock");
    const rollbackClaimPath = `${lockPath}.try-create-rollback-claim`;
    ...
    assert.throws(() => lock.withLock("BOOT-019", () => "should-not-run"));
...
test("FileArchitectureReviewTaskLock defers rather than proceeding as though nothing were there when reading the rollback claim fails transiently", () => {
  const root = mkdtempSync(join(tmpdir(), "ipt-architecture-review-lock-rollback-claim-read-failure-"));
  try {
    const lock = new FileArchitectureReviewTaskLock(root);
    const lockPath = join(root, "BOOT-019.lifecycle.lock");
    const rollbackClaimPath = `${lockPath}.try-create-rollback-claim`;
