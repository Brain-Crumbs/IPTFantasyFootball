...
test("FileQaReviewTaskLock's release() waits real wall-clock time for a competing reservation holder, not merely a bounded syscall count", () => {
  ...
  const root = mkdtempSync(join(tmpdir(), "ipt-qa-review-lock-real-time-reservation-wait-"));
  try {
    const lock = new FileQaReviewTaskLock(root);
    const lockPath = join(root, "BOOT-018.lifecycle.lock");
    const reservationPath = `${lockPath}.release-reservation`;
...
test("FileQaReviewTaskLock recovers an orphaned tryCreate() rollback claim (process crashed mid-rollback) rather than permanently displacing its owner", () => {
  ...
  // and happily create a brand-new stamp while the displaced content is
  // never recovered.
  const root = mkdtempSync(join(tmpdir(), "ipt-qa-review-lock-orphaned-rollback-claim-"));
  try {
    const lock = new FileQaReviewTaskLock(root);
    const lockPath = join(root, "BOOT-018.lifecycle.lock");
    ...
    // rename), and the displaced holder's own genuinely-stale stamp sits
    // orphaned at the rollback-claim path.
    writeFileSync(rollbackClaimPath, String(Date.now() - 10 * 60 * 1000), { encoding: "utf8" });
    const old = new Date(Date.now() - 10 * 60 * 1000);
...
test("FileQaReviewTaskLock never mistakes a live (fresh) tryCreate() rollback claim for an abandoned one", () => {
  const root = mkdtempSync(join(tmpdir(), "ipt-qa-review-lock-live-rollback-claim-"));
  try {
    const lock = new FileQaReviewTaskLock(root);
    const lockPath = join(root, "BOOT-018.lifecycle.lock");
    const rollbackClaimPath = `${lockPath}.try-create-rollback-claim`;
    ...
    // abandoned one.
    writeFileSync(rollbackClaimPath, "displaced-stamp", { encoding: "utf8" });
...
test("FileQaReviewTaskLock never races a second caller reaching the same stale conclusion against a still-live rollback-recovery claim", () => {
  const root = mkdtempSync(join(tmpdir(), "ipt-qa-review-lock-live-rollback-recovery-claim-"));
  try {
    const lock = new FileQaReviewTaskLock(root);
    const lockPath = join(root, "BOOT-018.lifecycle.lock");
    const rollbackClaimPath = `${lockPath}.try-create-rollback-claim`;
    ...
    writeFileSync(rollbackRecoveryClaimPath, "displaced-stamp", { encoding: "utf8" });
...
test("FileQaReviewTaskLock resumes an abandoned rollback-recovery claim (crashed after claiming, before finishing) rather than wedging the task forever", () => {
  ...
  const root = mkdtempSync(join(tmpdir(), "ipt-qa-review-lock-abandoned-rollback-recovery-claim-"));
  try {
    const lock = new FileQaReviewTaskLock(root);
    const lockPath = join(root, "BOOT-018.lifecycle.lock");
    const rollbackClaimPath = `${lockPath}.try-create-rollback-claim`;
    ...
    // A numeric epoch stamp old enough for reclaimIfStale to recognize the
    // restored lockPath content as stale and finish reclaiming it on the
    // same withLock() attempt's retry.
    const displacedStamp = String(Date.now() - 10 * 60 * 1000);
    writeFileSync(rollbackRecoveryClaimPath, displacedStamp, { encoding: "utf8" });
    const old = new Date(Date.now() - 10 * 60 * 1000);
...
test("FileQaReviewTaskLock fully drains a double-orphaned rollback claim (crashed between linking and removing the original) across successive acquisitions, never losing the displaced content", () => {
  ...
  const root = mkdtempSync(join(tmpdir(), "ipt-qa-review-lock-double-orphaned-rollback-claim-"));
  try {
    const lock = new FileQaReviewTaskLock(root);
    const lockPath = join(root, "BOOT-018.lifecycle.lock");
    const rollbackClaimPath = `${lockPath}.try-create-rollback-claim`;
    ...
    const displacedStamp = String(Date.now() - 10 * 60 * 1000);
    writeFileSync(rollbackClaimPath, displacedStamp, { encoding: "utf8" });
    writeFileSync(rollbackRecoveryClaimPath, displacedStamp, { encoding: "utf8" });
    const old = new Date(Date.now() - 10 * 60 * 1000);
    utimesSync(rollbackClaimPath, old, old);
    utimesSync(rollbackRecoveryClaimPath, old, old);

    assert.throws(() => lock.withLock("BOOT-018", () => "should-not-run"));
...
test("FileQaReviewTaskLock defers rather than proceeding as though nothing were there when reading the rollback claim fails transiently", () => {
  ...
  // permission errors.
  const root = mkdtempSync(join(tmpdir(), "ipt-qa-review-lock-rollback-claim-read-failure-"));
  try {
    const lock = new FileQaReviewTaskLock(root);
    const lockPath = join(root, "BOOT-018.lifecycle.lock");
    const rollbackClaimPath = `${lockPath}.try-create-rollback-claim`;
