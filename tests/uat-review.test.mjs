...
test("FileUatReviewTaskLock's release() waits real wall-clock time for a competing reservation holder, not merely a bounded syscall count", () => {
  ...
  const root = mkdtempSync(join(tmpdir(), "ipt-uat-review-lock-real-time-reservation-wait-"));
  try {
    const lock = new FileUatReviewTaskLock(root);
    const lockPath = join(root, "BOOT-020.lifecycle.lock");
    const reservationPath = `${lockPath}.release-reservation`;
...
test("FileUatReviewTaskLock recovers an orphaned tryCreate() rollback claim (process crashed mid-rollback) rather than permanently displacing its owner", () => {
  ...
  // never recovered.
  const root = mkdtempSync(join(tmpdir(), "ipt-uat-review-lock-orphaned-rollback-claim-"));
  try {
    const lock = new FileUatReviewTaskLock(root);
    const lockPath = join(root, "BOOT-020.lifecycle.lock");
...
test("FileUatReviewTaskLock never mistakes a live (fresh) tryCreate() rollback claim for an abandoned one", () => {
  const root = mkdtempSync(join(tmpdir(), "ipt-uat-review-lock-live-rollback-claim-"));
  try {
    const lock = new FileUatReviewTaskLock(root);
    const lockPath = join(root, "BOOT-020.lifecycle.lock");
    const rollbackClaimPath = `${lockPath}.try-create-rollback-claim`;
...
test("FileUatReviewTaskLock never races a second caller reaching the same stale conclusion against a still-live rollback-recovery claim", () => {
  const root = mkdtempSync(join(tmpdir(), "ipt-uat-review-lock-live-rollback-recovery-claim-"));
  try {
    const lock = new FileUatReviewTaskLock(root);
    const lockPath = join(root, "BOOT-020.lifecycle.lock");
    const rollbackClaimPath = `${lockPath}.try-create-rollback-claim`;
...
test("FileUatReviewTaskLock resumes an abandoned rollback-recovery claim (crashed after claiming, before finishing) rather than wedging the task forever", () => {
  ...
  const root = mkdtempSync(join(tmpdir(), "ipt-uat-review-lock-abandoned-rollback-recovery-claim-"));
  try {
    const lock = new FileUatReviewTaskLock(root);
    const lockPath = join(root, "BOOT-020.lifecycle.lock");
    const rollbackClaimPath = `${lockPath}.try-create-rollback-claim`;
...
test("FileUatReviewTaskLock fully drains a double-orphaned rollback claim (crashed between linking and removing the original) across successive acquisitions, never losing the displaced content", () => {
  ...
  const root = mkdtempSync(join(tmpdir(), "ipt-uat-review-lock-double-orphaned-rollback-claim-"));
  try {
    const lock = new FileUatReviewTaskLock(root);
    const lockPath = join(root, "BOOT-020.lifecycle.lock");
    const rollbackClaimPath = `${lockPath}.try-create-rollback-claim`;
    ...
    assert.throws(() => lock.withLock("BOOT-020", () => "should-not-run"));
...
test("FileUatReviewTaskLock defers rather than proceeding as though nothing were there when reading the rollback claim fails transiently", () => {
  const root = mkdtempSync(join(tmpdir(), "ipt-uat-review-lock-rollback-claim-read-failure-"));
  try {
    const lock = new FileUatReviewTaskLock(root);
    const lockPath = join(root, "BOOT-020.lifecycle.lock");
    const rollbackClaimPath = `${lockPath}.try-create-rollback-claim`;
